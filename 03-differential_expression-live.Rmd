...
integrated_sce <- readRDS(integrated_sce_file)
...
names(integrated_sce@assays) # or assayNames(integrated_sce)
```
...
scater::plotUMAP(integrated_sce,
                 dimred = "fastmnn_UMAP",
                 color_by = "diagnosis_group",
                 point_size = 0.5,
                 point_alpha = 0.2
) +
  scale_color_brewer(palette = "Dark2", name = 'diagnosis_group')
  ggtitle("UMAP by diagnosis group")
```
...
pb_counts <- DelayedArray::colsum(counts_mtx, groups) # different from colSums
                                                      # this sums specific sets of columns across a row
pb_counts
```
...
filter_pb_sce <- pb_sce[, which(pb_sce$ncells >= 10)]
```
...
```{r plotPCA, live=TRUE}
DESeq2::plotPCA(normalized_object,
                intgroup = "diagnosis_group")
```
...
# Plot to estimate how well the model fit
plotDispEsts(deseq_object) # plotting average normalized count per gene x dispersion for that gene
```
...
shrink_results <- DESeq2::lfcShrink(
  ...
  res = deseq_results,
  coef = 2,
  type = "apeglm" # default recommended
)
head(shrink_results)
```
...
scater::plotReducedDim(
  ...
  dimred = "fastmnn_UMAP",
  color_by = "ENSG00000196090",
  point_size = 0.5,
  point_alpha = 0.4,
  other_fields = "diagnosis_group"
) +
  facet_grid(cols = vars(diagnosis_group)) ### what is this doing?????
```
