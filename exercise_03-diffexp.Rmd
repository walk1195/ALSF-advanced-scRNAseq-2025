...
readr::write_tsv(deseq_mesoderm_results, deseq_mesoderm_file)
```

```{r}
# GEW: Why use readr::write_tsv as opposed to write.table ?
```
...
table(deseq_results_all$significance_type)
```
...
# I want to look at how much the DE results change when we perform shrinkage
# Let's first grab the unshrunk result

# Plotting PAX3 - Mesoderm only gene
scater::plotUMAP(
  rms_sce,
  color_by = "ENSG00000135903",
  other_fields = "celltype_broad"
) +
