...
output_dir <- file.path("data,", "pancreas", "integrated")
...
paths <- file.path(
  input_dir, # provide start of file path string
  glue::glue("{donors}_sce.rds") # provide vector with donor IDs
)
...
format_sce <- function(
  ...
  colnames(sce) <- glue::glue("{donor_id}-{colnames(sce)}")
...
If we want to use those names in our internal function, we can automatically have them passed in as the second argument by using  `purrr::imap()` (the `i` stands for "index" here) without having to formally write it out.
...
merged_pca <- batchelor::multiBatchPCA(
  merged_sce,
  subset.row = hv_genes,
...
merged_sce$cluster_unintegrated <- scran::clusterCells(
  merged_sce,
  use.dimred = "PCA",
...
#  scale_color_brewer(palette = "Dark2", name = "sample") +
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1))) +
  ggtitle("UMAP by cluster (unintegrated)")
```
...
scater::plotUMAP(merged_sce,
  ...
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1))) +
  ggtitle("UMAP by sample (integrated)")
```
...
scater::plotUMAP(shuffled_sce,
  ...
  scale_color_brewer(palette = "Dark2", name = "sample") +
  guides(color = guide_legend(override.aes = list(size = 3, alpha = 1))) +
  ggtitle("UMAP by sample (integrated and shuffled)")
```
...
merged_sce$cluster_integrated <- scran::clusterCells(
  merged_sce,
  use.dimred = "fastmnn_PCA",
  BLUSPARAM = bluster::NNGraphParam()
)
...
scater::plotUMAP(merged_sce,
                 dimred = "fastmnn_UMAP",
                 color_by = "cluster_integrated",
                 point_size = 0.5,
                 point_alpha = 0.2) +
# add more CVD-friendly color scale and legend title
#scale_color_brewer(palette = "Dark2", name = "Clusters") +
guides(color = guide_legend(override.aes = list(size = 3, alpha = 1))) +
ggtitle("UMAP by cluster (integrated and shuffled)")
```
